...
class BinaryTreeNasabah {
    private:
        ...
        Nasabah* cariByIdNasabahHelper(TreeNode* node, const string& IdNasabah) {
            if (!node) return nullptr;
            if (node->data.IdNasabah == IdNasabah) return &node->data;

            Nasabah* left = cariByIdNasabahHelper(node->left, IdNasabah);
            if (left) return left;

            return cariByIdNasabahHelper(node->right, IdNasabah);
        }
    ...
    public:
        ...
        void insertTree(const Nasabah& nasabah) {
            root = insert(root, nasabah);
        }
        ...
        Nasabah* findNasabahById(string idNasabah) {
            return cariByIdNasabahHelper(root, idNasabah);
        }

        bool getStatusAktifById(string idNasabah) {
            Nasabah* nasabah = findNasabahById(idNasabah);
            if (nasabah != nullptr) {
                return nasabah->aktif;
            } else {
                return false;
            }
        }

        void updateStatusAktifById(string idNasabah, bool status) {
            Nasabah* nasabah = findNasabahById(idNasabah);
            if (nasabah != nullptr) {
                nasabah->aktif = status;
            }
        }   
};